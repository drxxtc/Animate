...
x_input, y_input, speed_input, speed_point = None, None, None, None
...
def animate_ellipse(Npoints=5,fps=30):
    ...
    ellipse1=Ellipse()
    list_points=[Point(np.array([0.5,0.5]),np.random.rand(1,2)[0]) for i in range(Npoints)]
    ...
    anim = FuncAnimation(fig, drawframe,frames=2000, interval=300/fps, blit=True)
    return anim
...
class Bezier_curve:
    def __init__(self,N=12,speed=0.1,speedturn=0.2):
...
def animate_bezier(Npoints=12,fps=30):
    plt.style.use('dark_background')
    ...
    b=Bezier_curve(Npoints,speed=speed_input)
    list_points = [Point(b.points[i],normalize(np.random.rand(1,2)[0]),speed=speed_point*0.01) for i in range(len(b.points))]
...
def main(speed_input, speed_point, choice, num_point):
